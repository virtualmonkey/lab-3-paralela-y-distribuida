...
void Generate_random_vector(double local_a[], int local_n, int n, char vec_name[], 
int my_rank, MPI_Comm comm);
...
void Parallel_vector_dot(double local_x[], double local_y[], 
      double local_z[], int local_n, double* local_dotProduct);
...
#  endif
   ...
   Generate_random_vector(local_x, local_n, n, "x", my_rank, comm);
   Print_vector(local_x, local_n, n, "x is", my_rank, comm);
   Generate_random_vector(local_y, local_n, n, "y", my_rank, comm);
   ...
   Print_vector(local_z, local_n, n, "Resultado en z", my_rank, comm);
...
void Parallel_vector_dot(
   ...
   for (local_j = 0; local_j < local_n; local_j++){
      printf("z[local_j] -> %f ", local_z[local_j]);
